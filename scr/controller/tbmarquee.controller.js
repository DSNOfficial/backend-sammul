...
const getList = async (req, res) => {
    try {
        ...
        let sql = "SELECT * FROM tbmarquee WHERE 1=1";
        let sqlWhere = "";
        let param = {};
        ...
        logError("tbmarquee.getList", err, res);
    }
}
...
const create = async (req, res) => {
    try {
        ...
        const sql = "INSERT INTO tbmarquee ( title, description,Status,Image) VALUES ( :title,:description,:Status ,:Image)";
        const param = {
        ...
        logError("tbmarquee.create", err, res);
    }
}
...
const update = async (req, res) => {
    try {
        ...
        const [dataInfo] = await db.query("SELECT * FROM tbmarquee WHERE id=:id", { id: id });
        if (dataInfo.length > 0) {
            const sql = "UPDATE tbmarquee SET  title=:title, Image=:Image,description=:description,Status=:Status WHERE id = :id";
            const [data] = await db.query(sql, param);
        ...
        logError("tbmarquee.update", err, res);
    }
}
...
const remove = async (req, res) => {
    try {
        ...
        const [dataInfo] = await db.query("SELECT * FROM tbmarquee WHERE id=:id", param);
        if (dataInfo.length > 0) {
            const sql = "DELETE FROM tbmarquee WHERE id = :id";
            const [data] = await db.query(sql, param);
            if (data.affectedRows) {
        ...
        logError("tbmarquee.remove", err, res);
    }
}
