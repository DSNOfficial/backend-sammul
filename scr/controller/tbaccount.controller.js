...
const getList = async (req, res) => {
    try {
        ...
        let sql = "SELECT * FROM tbaccount WHERE 1=1";
        let sqlWhere = "";
        let param = {};
        ...
        logError("tbaccount.getList", err, res);
    }
}
...
const getOne = async (req, res) => {
    try {
      const sql = "SELECT * FROM tbaccount WHERE id = :id";
      const param = {
...
const create = async (req, res) => {
    try {
        ...
        const sql = "INSERT INTO tbaccount ( title, description,Status,Image) VALUES ( :title,:description,:Status ,:Image)";
        const param = {
        ...
        logError("tbaccount.create", err, res);
    }
}
...
const update = async (req, res) => {
    try {
        ...
        const [dataInfo] = await db.query("SELECT * FROM tbaccount WHERE id=:id", { id: id });
        if (dataInfo.length > 0) {
            const sql = "UPDATE tbaccount SET  title=:title, Image=:Image,description=:description,Status=:Status WHERE id = :id";
            const [data] = await db.query(sql, param);
        ...
        logError("tbaccount.update", err, res);
    }
}
...
const remove = async (req, res) => {
    try {
        ...
        const [dataInfo] = await db.query("SELECT * FROM tbaccount WHERE id=:id", param);
        if (dataInfo.length > 0) {
            const sql = "DELETE FROM tbaccount WHERE id = :id";
            const [data] = await db.query(sql, param);
            if (data.affectedRows) {
        ...
        logError("tbaccount.remove", err, res);
    }
}
